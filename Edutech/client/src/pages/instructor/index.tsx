...
function LevelBadge({ level }: { level: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold border capitalize ${colors[level] ?? "bg-muted"}`}
    >
      {level}
    </span>
  );
...
function CourseRow({
  ...
  const [deleteLessonTarget, setDeleteLessonTarget] = useState<Lesson | null>(
    null,
  );
  ...
  const handleDeleteLesson = () => {
    ...
    deleteLessonMutation.mutate(
      { id: deleteLessonTarget.id },
      {
        onSuccess: () => {
          queryClient.invalidateQueries({
            queryKey: getListLessonsQueryKey(course.id),
          });
  ...
  return (
    <>
            ...
            <div className="flex items-start gap-4 p-5">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-5 mt-3 text-xs text-muted-foreground">
                  ...
                  <div className="flex items-center gap-1.5">
                    <ListVideo className="h-3.5 w-3.5 text-primary/60" />
                    <span className="font-medium text-foreground">
                      {course.lessonCount}
                    </span>{" "}
                    lessons
      ...
      <AlertDialog
        ...
        onOpenChange={(o: boolean) => !o && setDeleteLessonTarget(null)}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete this lesson?</AlertDialogTitle>
            <AlertDialogDescription>
              "{deleteLessonTarget?.title}" will be permanently removed. This
              cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
...
export default function InstructorPortal() {
  ...
  const handleDeleteCourse = () => {
    ...
    deleteCourseMutation.mutate(
      { id: deleteCourseTarget.id },
      {
        onSuccess: () => {
          queryClient.invalidateQueries({ queryKey: getListCoursesQueryKey() });
          toast.success(`"${deleteCourseTarget.title}" deleted`);
  ...
  const stats = {
    ...
    totalLessons:
      courses?.reduce((sum: number, c: any) => sum + (c.lessonCount ?? 0), 0) ??
      0,
    totalEnrollments:
      courses?.reduce(
        (sum: number, c: any) => sum + (c.enrollmentCount ?? 0),
        0,
      ) ?? 0,
  };
  ...
  return (
    <PageTransition>
          ...
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              ...
              <p className="text-muted-foreground">
                Manage your courses, lessons, and student content from one
                place.
              </p>
            </div>
            <Button
        ...
        <div className="flex items-center justify-between mb-6">
          <CardHeader className="p-0">
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Click "Lessons" on any course to expand and manage its curriculum.
            </p>
          </CardHeader>
        </div>
        {isLoading ? (
      ...
      <AlertDialog
        ...
        onOpenChange={(o: boolean) => !o && setDeleteCourseTarget(null)}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete this course?</AlertDialogTitle>
            <AlertDialogDescription>
              <strong>"{deleteCourseTarget?.title}"</strong> and all its lessons
